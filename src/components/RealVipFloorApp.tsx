...
export default function RealVipFloorApp() {
  ...
  async function command(kind: "check_in" | "seat_extension", reservation: Reservation) {
    setBusy(true);
    const payload = kind === "seat_extension" ? { extendMinutes: 30, reason: "vip_floor_operator" } : { reason: "vip_floor_operator" };
    ...
    if (response.ok) { setMessage(kind === "check_in" ? "チェックインを保存しました" : "30分延長を保存しました"); await loadBoard(); }
