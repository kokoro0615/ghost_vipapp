...
const tabs = ["概要", "ゲスト", "接客", "決済", "メモ", "履歴"] as const;
      ...
      {reservation ? (
        <>
          ...
          <div className={styles.inspectorPanel} role="tabpanel" id={`${instance}-${tab}-panel`} aria-labelledby={`${instance}-${tab}-tab`}>
            {tab === "概要" ? <dl className={styles.detailList}>
            ...
            {tab === "ゲスト" ? <div className={styles.detailStack}><p className={styles.maskedName}><UserRound size={17} />{reservation.guestLabel}</p><p>この端末では業務に必要なマスク済み表示名のみ扱います。</p><small>電話・メール・個人情報のエクスポートは対象外です。</small></div> : null}
            {tab === "接客" ? <div className={styles.detailStack}><p><StatusIcon size={16} /> {meta.label}</p><p>元データ: {reservation.sourceLabel}</p><p>席ロック: {table?.operationalLocked ? table.lockReason : "なし"}</p><p>フラグ: {reservation.flags.join(", ") || "なし"}</p></div> : null}
            {tab === "決済" ? <div className={styles.detailStack}><p><CircleDollarSign size={16} /> 対象外</p><p>決済・返金操作はこのVIP Floorアプリでは行いません。</p><small>必要な場合はGHOST本体の承認済み手順を使用してください。</small></div> : null}
            {tab === "メモ" ? <div className={styles.noteList}>{notes.length ? notes.map((note) => <article key={note.id}><strong><NotebookPen size={14} />{note.pinned ? "固定メモ" : "メモ"}</strong><p>{note.body}</p><small>v{note.version} / {note.kind}</small></article>) : <p>メモはありません。</p>}</div> : null}
            {tab === "履歴" ? <ol className={styles.historyList}>{history.map((item) => <li key={item.id}><span>{new Intl.DateTimeFormat("ja-JP", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Tokyo" }).format(new Date(item.at))}</span><div><strong>{item.label}</strong><p>{item.detail}</p><small>{item.actor}</small></div></li>)}</ol> : null}
          </div>
        ...
        <div className={styles.noSelection}><MapPin size={24} /><h2>席または予約を選択</h2><p>フロアノード、時間軸バー、一覧行、キュー項目のいずれからでも同じインスペクターを開けます。</p></div>
      )}
    </aside>
  );
}
