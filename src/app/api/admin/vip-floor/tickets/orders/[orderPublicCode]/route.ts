...
export async function GET(
  request: Request,
  context: { params: Promise<{ orderPublicCode: string }> },
) {
  ...
  const { orderPublicCode } = await context.params;
  if (!ORDER_PUBLIC_CODE_PATTERN.test(orderPublicCode)) {
    return ticketOperationsJson({ ok: false, error: "invalid_order_public_code" }, 400);
  }
  ...
  if (!scope) return ticketOperationsJson({ ok: false, error: "invalid_scope" }, 400);
  ...
  const response = await ticketOperationsFetch(
    `/api/admin/v2/tickets/orders/${encodeURIComponent(orderPublicCode)}?scope=${scope}`,
  ...
  try {
    return ticketOperationsJson(projectTicketOperationsOrder(payload), response.status);
  } catch {
