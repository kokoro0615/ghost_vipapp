...
function requiredFlag(argv, name) {
  ...
  if (indexes.length !== 1 || indexes[0] === argv.length - 1) {
    throw new Error(`active_candidate_flag_invalid:${name}`);
  }
...
function parseEnvironment(source) {
  ...
  for (const rawLine of source.split(/\r?\n/u)) {
    ...
    if (!match || values.has(match[1])) throw new Error("active_candidate_env_invalid");
...
function requireNoStore(headers, stage) {
  ...
  if (!/(?:^|,)\s*(?:private\s*,\s*)?(?:no-cache\s*,\s*)?no-store(?:\s*,|$)/iu.test(cacheControl)) {
    throw new Error(`active_candidate_no_store_missing:${stage}`);
  }
}

function parseBoundedJson(source, stage) {
  if (Buffer.byteLength(source, "utf8") > 65_536) {
    throw new Error(`active_candidate_body_too_large:${stage}`);
  }
  try {
    ...
    throw new Error(`active_candidate_body_invalid:${stage}`);
  }
}
...
async function requestCandidate({
  ...
  if (result?.error || result?.status !== 0) {
    throw new Error(`active_candidate_request_failed:${stage}`);
  }
  await Promise.all([
...
function assertPage(response) {
  if (response.status !== 200) throw new Error(`active_candidate_page_http_${response.status}`);
  if (!/^referrer-policy:\s*no-referrer\s*$/imu.test(response.headers)) {
    throw new Error("active_candidate_referrer_policy_missing");
  }
  if (!/^x-robots-tag:\s*[^\r\n]*noindex/imu.test(response.headers)) {
    throw new Error("active_candidate_robots_policy_missing");
  }
}

function assertSession(response) {
  if (response.status !== 200) throw new Error(`active_candidate_session_http_${response.status}`);
  const value = parseBoundedJson(response.body, "session");
  if (value.ok !== true || value.mode !== "owner" || value.role !== "owner") {
    throw new Error("active_candidate_owner_session_invalid");
  }
}

function assertCapabilities(response) {
  if (response.status !== 200) throw new Error(`active_candidate_capabilities_http_${response.status}`);
  const value = parseBoundedJson(response.body, "capabilities");
  if (
    value.ok !== true
    || value.capabilities?.managerOperationsEnabled !== true
    || value.capabilities?.refundReviewEnabled !== true
    || value.readiness?.managerOperations !== "ready"
    || value.readiness?.refundReview !== "ready"
  ) throw new Error("active_candidate_capabilities_mismatch");
  return value;
}

function assertLogout(response) {
  if (response.status !== 200) throw new Error(`active_candidate_logout_http_${response.status}`);
  const value = parseBoundedJson(response.body, "logout");
  if (value.ok !== true) throw new Error("active_candidate_logout_invalid");
}

export async function runVipActiveCandidateSmoke(
  { repoRoot = process.cwd(), deploymentId, envFile },
  runtime = createProductionRuntime(),
) {
  if (!DEPLOYMENT_ID.test(deploymentId ?? "")) throw new Error("active_candidate_deployment_id_invalid");
  const credentials = await readOwnerCredentials(envFile, runtime);
  const tempRoot = await runtime.mkdtemp(path.join(runtime.tmpdir(), "ghost-vip-active-smoke-"));
...
async function mutateProtectionBypass(secret, operation) {
  ...
  if (operation === "acquire") {
    if (entries.length !== 0) throw new Error("active_candidate_bypass_already_present");
  } else if (entries.length === 0) {
    return;
  } else if (entries.length !== 1 || entries[0][0] !== secret) {
    throw new Error("active_candidate_bypass_cleanup_ambiguous");
  }
  const url = new URL(
  ...
  if (!response.ok) throw new Error(`active_candidate_bypass_${operation}_http_${response.status}`);
  ...
  ) throw new Error(`active_candidate_bypass_${operation}_readback_failed`);
}
...
async function main() {
  ...
  const result = await runVipActiveCandidateSmoke({ deploymentId, envFile });
  console.log(JSON.stringify(result));
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((error) => {
    console.error(error instanceof Error ? error.message : "active_candidate_smoke_failed");
    process.exitCode = 1;
  });
}
