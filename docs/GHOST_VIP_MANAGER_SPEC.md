...
- キャンセル料・返金の請求操作（取消そのものは全経路で対象。2026-09-19 Owner決定により、
  オンライン予約の直前キャンセル料はStripeダッシュボードで請求する）
- SMS、LINE通知
- PDF、印刷、CSV、顧客export
- 多店舗切替
- 英語UI
...
- 既存6操作と予約取消、Walk-in、Waitlist、ブロック、受付停止、担当卓がOwner権限で動作する。
- 予約取消は店頭・電話・管理者・オンラインのすべての経路で表示し、理由・影響の2段階確認、
  expectedVersion、idempotency、監査ID、席解放を満たす。返金・カード請求・通知・物理削除は
  行わない。
